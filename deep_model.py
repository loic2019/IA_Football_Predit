...
def train(X: np.ndarray, y: np.ndarray, epochs: int = 200, patience: int = 15) -> dict:
    ...
    # pour la justification complète (Partie N : jamais mélanger passé et
    # futur). X arrive déjà trié chronologiquement depuis
    # feature_engineering.builder.build_training_matrix ; les derniers 20%
    # (les plus récents) servent de validation, sans permutation aléatoire.
    n = len(X_norm)
