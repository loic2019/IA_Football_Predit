...
from ai_config import is_ai_enabled, ASSISTANT_NAME, get_active_provider
from voice_io import transcribe_audio, synthesize_speech
from voice_config import is_precise_stt_enabled
...
def _process_user_message(user_input: str, avatar_slot):
    """Ajoute le message utilisateur, génère la réponse de Max, et prépare
    l'audio de la réponse si la voix est activée. Commun au texte et au vocal."""
    ...
    if st.session_state.get("voice_reply_enabled"):
        st.session_state.last_bot_audio = synthesize_speech(reply)

    if st.session_state.last_bot_audio:
        render_avatar(avatar_slot, "speaking", audio_bytes=st.session_state.last_bot_audio)
    else:
...
def render():
    ...
    if is_ai_enabled():
        provider_label = "Groq / Llama" if get_active_provider() == "groq" else "Claude"
        st.caption(f"🟢 Mode IA activé ({provider_label}) — réponses naturelles, ancrées sur tes vraies données")
    else:
        st.caption(
            "🟡 Mode basique (mots-clés) — configure ANTHROPIC_API_KEY ou GROQ_API_KEY pour activer "
            "les réponses naturelles et dynamiques (voir ai_config.py)"
        )

    if is_precise_stt_enabled():
        st.caption("🎙️ Reconnaissance vocale précise activée (Whisper) — comprend bien le langage parlé/argot")
    else:
        st.caption(
            "🎙️ Reconnaissance vocale en mode gratuit (moins précis sur l'argot) — configure "
            "OPENAI_API_KEY pour la version précise (voir voice_config.py)"
        )
    ...
    st.session_state.voice_reply_enabled = st.toggle(
        "🔊 Max te répond aussi à voix haute", value=st.session_state.voice_reply_enabled
    )
    ...
    st.caption("🎙️ Parle à Max :")
    audio_value = st.audio_input("Enregistre ton message vocal", label_visibility="collapsed")
    if audio_value is not None:
        ...
        if st.session_state.get("last_audio_id") != audio_id:
            ...
            if transcribed:
                _process_user_message(transcribed, avatar_slot)
                st.rerun()
            else:
                render_avatar(avatar_slot, "idle")
                st.warning("Je n'ai rien compris à cet enregistrement, réessaie ou écris ton message ci-dessous.")
    ...
    if submitted and user_input.strip():
        _process_user_message(user_input, avatar_slot)
        st.rerun()
    ...
    for col, question in zip(cols, quick_questions):
        if col.button(question, width="stretch"):
            _process_user_message(question, avatar_slot)
            st.rerun()
