...
def render():
    ...
    if not coupon.get("selections"):
        ...
        fallback_count = coupon.get("fallback_count", 0)
        if value_count == 0 and fallback_count > 0:
            st.warning(
                "⚠️ Aucun match du jour n'offre d'avantage statistique réel par rapport aux cotes "
                "du bookmaker (valeur espérée négative sur toute la sélection, en moyenne "
                f"{coupon.get('avg_expected_value', 0):.0%}). Ce coupon est rempli quand même avec les "
                "picks les plus confiants, mais **une confiance élevée ne veut pas dire un bon pari** — "
                "la cote proposée ne compense pas le risque. À prendre comme information, pas comme "
                "recommandation de mise."
            )
        elif fallback_count > 0:
            st.caption(
                f"ℹ️ {value_count} sélection(s) avec un avantage statistique réel, "
                f"{fallback_count} ajoutée(s) sans edge démontré pour compléter le coupon."
            )
        else:
            st.caption(f"✅ Les {value_count} sélections ont toutes une valeur espérée positive par rapport aux cotes du marché.")
        ...
        if top_pick.get("model_breakdown"):
            with st.expander(f"🧠 Pourquoi ce pronostic : {top_pick['home']} vs {top_pick['away']} ?"):
                ...
                for model_name, detail in top_pick["model_breakdown"].items():
                    ...
                    weight_txt = f" — poids {weight:.0%}" if isinstance(weight, (int, float)) else ""
                    st.markdown(
                        f"**{model_name}**{weight_txt} — "
                        f"1: {probs.get('1', 0):.0%} · X: {probs.get('X', 0):.0%} · 2: {probs.get('2', 0):.0%}"
                    )

    st.markdown("### 🎫 Ticket 15 matchs")
    st.caption(
        "Un second ticket, plus large que le coupon conseillé (8 matchs) — seuils de "
        "confiance/cote volontairement plus souples pour pouvoir en réunir 15."
    )
    coupon15 = snapshot.get("coupon15")
    if not coupon15 or not coupon15.get("selections"):
        st.info("Pas encore assez de matchs valables pour former un ticket de 15 sélections.")
    else:
        col_table15, col_slip15 = st.columns([3, 1])
        with col_table15:
            st.dataframe(
                _prediction_rows(coupon15["selections"]),
                hide_index=True,
                column_config={
                    "Confiance": st.column_config.ProgressColumn(
                        "Confiance", min_value=0, max_value=1, format="percent"
        ...
        with col_slip15:
            st.metric("Cote totale", f"{coupon15['total_cote']:.2f}")
            st.metric("Confiance moyenne", f"{coupon15['avg_confidence']:.0%}")
            if st.button("💾 Sauvegarder ce ticket", key="save_coupon15_btn", width="stretch"):
                from coupon_tracker import save_daily_coupon
                result = save_daily_coupon(coupon15, force=True)
                if result.get("saved"):
                    st.success(f"✅ Ticket sauvegardé (id {result['coupon_id']}).")
                else:
                    st.warning(f"ℹ️ {result.get('reason', 'Non sauvegardé')}")

    st.markdown("---")
    st.markdown("### 🎟️🎟️ Combinaison de coupons (10 matchs chacun)")
    st.caption(
        "Tous les matchs valables sont répartis en plusieurs tickets de 10 sélections, "
        "classés du plus sûr (confiance la plus haute) au plus risqué — plutôt qu'un seul "
        "coupon limité aux 8 meilleurs."
    )
    multi_coupons = snapshot.get("multi_coupons", [])
    if not multi_coupons:
        st.info("Pas encore assez de matchs valables pour former un coupon complet de 10 (il en faut au moins 10).")
    else:
        tabs = st.tabs([c["label"] for c in multi_coupons])
        for tab, mc in zip(tabs, multi_coupons):
            with tab:
                c1, c2 = st.columns([3, 1])
                with c1:
                    st.dataframe(
                        _prediction_rows(mc["selections"]),
                        hide_index=True,
                        column_config={
                            "Confiance": st.column_config.ProgressColumn(
                                "Confiance", min_value=0, max_value=1, format="percent"
                ...
                with c2:
                    st.metric("Cote totale", f"{mc['total_cote']:.2f}")
                    st.metric("Confiance moyenne", f"{mc['avg_confidence']:.0%}")
                    if st.button("💾 Sauvegarder ce coupon", key=f"save_multi_{mc['label']}"):
                        from coupon_tracker import save_daily_coupon
                        result = save_daily_coupon(mc, force=True)
                        if result.get("saved"):
                            st.success(f"✅ Sauvegardé (id {result['coupon_id']}).")
                        else:
                            st.warning(f"ℹ️ {result.get('reason', 'Non sauvegardé')}")
...
def _render_coupon_history_section():
    ...
    c_settle, c_diag = st.columns(2)
    if c_settle.button("🔄 Vérifier les coupons en attente maintenant", key="settle_now_btn"):
    ...
    if c_diag.button("🔍 Diagnostiquer les coupons en attente", key="diagnose_pending_btn"):
        import json as _json
        import sqlite3 as _sqlite3
        from coupon_tracker import _lookup_result, _connect

        conn = _connect()
        pending_rows = conn.execute("SELECT * FROM coupon_history WHERE status = 'pending'").fetchall()
        if not pending_rows:
            st.info("Aucun coupon en attente actuellement.")
        for row in pending_rows:
            matches = _json.loads(row["matches_json"])
            with st.expander(f"Coupon du {row['coupon_date']} ({len(matches)} matchs)"):
                for m in matches:
                    res = _lookup_result(conn, m["match_id"], m.get("home", ""), m.get("away", ""), m.get("start_time", ""))
                    if res is not None:
                        st.caption(f"✅ {m['home']} vs {m['away']} — résultat trouvé ({res['result']})")
                    else:
                        st.caption(f"⏳ {m['home']} vs {m['away']} — **résultat introuvable pour l'instant** (aucune des 3 sources ne le connaît)")
        conn.close()
    ...
    if history:
        ...
        for c in history:
            rows.append({
                ...
                "Bons pronostics": str(c.get("hits")) if c.get("hits") is not None else "—",
