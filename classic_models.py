...
def _split(X, y, val_fraction: float = 0.2):
    """
    Split CHRONOLOGIQUE (pas aléatoire) — voir ml_models.tree_models._temporal_split
    pour la justification complète (Partie N : "ne mélange pas aléatoirement
    passé et futur"). X/y arrivent déjà triés chronologiquement depuis
    feature_engineering.builder.build_training_matrix ; on se contente de
    découper séquentiellement au lieu d'appeler
    sklearn.model_selection.train_test_split (qui mélange par défaut).
    """
    n = len(X)
