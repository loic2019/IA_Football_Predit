...
def init_community_db() -> None:
    ...
    conn.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_wallet_bets_user ON wallet_bets(user_id, status);

        CREATE TABLE IF NOT EXISTS wallet_coupon_bets (
            id           INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id      INTEGER NOT NULL REFERENCES users(id),
            engine       TEXT NOT NULL,
            label        TEXT,
            total_cote   REAL NOT NULL,
            stake        REAL NOT NULL,
            status       TEXT NOT NULL DEFAULT 'pending',
            placed_at    TEXT NOT NULL,
            settled_at   TEXT
        );
        CREATE INDEX IF NOT EXISTS idx_wallet_coupon_bets_user ON wallet_coupon_bets(user_id, status);

        CREATE TABLE IF NOT EXISTS wallet_coupon_legs (
            id             INTEGER PRIMARY KEY AUTOINCREMENT,
            coupon_bet_id  INTEGER NOT NULL REFERENCES wallet_coupon_bets(id),
            match_id       TEXT,
            home           TEXT,
            away           TEXT,
            league         TEXT,
        ...
        CREATE INDEX IF NOT EXISTS idx_wallet_coupon_legs_bet ON wallet_coupon_legs(coupon_bet_id);
        """
    )
...
DEFAULT_WALLET_BALANCE = 100_000  # Frs, montant de départ pour le mode "gestion de budget"


def get_or_create_wallet(user_id: int, initial: float = DEFAULT_WALLET_BALANCE) -> dict:
    """Portefeuille virtuel de l'utilisateur (créé au 1er accès avec le solde
    de départ). Sert à s'entraîner à gérer un budget de paris sans argent réel."""
    conn = _connect()
    try:
        ...
        conn.execute(
            "INSERT INTO wallets (user_id, balance, initial_balance, created_at, updated_at) VALUES (?,?,?,?,?)",
            (user_id, initial, initial, now, now),
        )
        conn.commit()
        return {"user_id": user_id, "balance": initial, "initial_balance": initial, "created_at": now, "updated_at": now}
    finally:
        conn.close()


def reset_wallet(user_id: int, initial: float = DEFAULT_WALLET_BALANCE) -> None:
    """Remet le portefeuille à zéro (nouveau départ), sans toucher à l'historique
    des paris déjà passés (gardés pour trace, mais n'affectent plus le solde)."""
    conn = _connect()
    try:
        now = datetime.now().isoformat()
        conn.execute(
            "UPDATE wallets SET balance = ?, initial_balance = ?, updated_at = ? WHERE user_id = ?",
            (initial, initial, now, user_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def place_wallet_bet(user_id: int, engine: str, match: dict, prediction: str, cote: float, stake: float) -> dict:
    """Place un pari virtuel : débite le solde immédiatement (comme un vrai pari),
    le pari reste 'pending' jusqu'au règlement automatique du résultat réel."""
    wallet = get_or_create_wallet(user_id)
    if stake <= 0:
        return {"placed": False, "reason": "Mise invalide."}
    if stake > wallet["balance"]:
        return {"placed": False, "reason": f"Solde insuffisant ({wallet['balance']:.0f} Frs disponibles)."}

    conn = _connect()
    try:
        now = datetime.now().isoformat()
        conn.execute(
            """INSERT INTO wallet_bets (user_id, engine, match_id, home, away, league, prediction, cote, stake, status, placed_at)
               VALUES (?,?,?,?,?,?,?,?,?, 'pending', ?)""",
            (user_id, engine, str(match.get("id") or match.get("match_id") or ""), match.get("home"),
             match.get("away"), match.get("league"), prediction, cote, stake, now),
        )
        conn.execute(
        ...
        conn.commit()
        return {"placed": True, "new_balance": wallet["balance"] - stake}
    finally:
        conn.close()


def settle_wallet_bets() -> dict:
    """Règle tous les paris virtuels en attente dont le résultat réel est
    maintenant connu (réutilise le même filet de résolution à 3 niveaux que
    les vrais coupons — voir coupon_tracker._lookup_result). Crédite/décrédite
    le solde de chaque utilisateur en conséquence."""
    from coupon_tracker import _lookup_result

    conn = _connect()
    settled = won = lost = 0
    try:
        pending = conn.execute("SELECT * FROM wallet_bets WHERE status = 'pending'").fetchall()
        result_conn = sqlite3.connect("congobet.db")
        result_conn.row_factory = sqlite3.Row
        for bet in pending:
            result = _lookup_result(result_conn, bet["match_id"], bet["home"] or "", bet["away"] or "", "")
            if not result or not result.get("result"):
                continue

            now = datetime.now().isoformat()
            won_bet = result["result"] == bet["prediction"]
            status = "won" if won_bet else "lost"
            payout = bet["stake"] * bet["cote"] if won_bet else 0.0

            conn.execute(
                "UPDATE wallet_bets SET status = ?, settled_at = ? WHERE id = ?",
                (status, now, bet["id"]),
            )
            if payout > 0:
                conn.execute(
                    "UPDATE wallets SET balance = balance + ?, updated_at = ? WHERE user_id = ?",
                    (payout, now, bet["user_id"]),
                )
            settled += 1
            won += 1 if won_bet else 0
            lost += 0 if won_bet else 1
        result_conn.close()
        conn.commit()
    finally:
        conn.close()
...
def place_wallet_coupon_bet(user_id: int, engine: str, label: str, selections: list[dict], total_cote: float, stake: float) -> dict:
    """Place un pari virtuel sur un COUPON ENTIER (plusieurs matchs combinés) :
    débite la mise une seule fois, la cote totale s'applique globalement.
    Le pari n'est gagné QUE SI TOUS les matchs du ticket sont corrects — comme
    un vrai coupon combiné chez un bookmaker."""
    wallet = get_or_create_wallet(user_id)
    if stake <= 0:
        return {"placed": False, "reason": "Mise invalide."}
    if stake > wallet["balance"]:
        return {"placed": False, "reason": f"Solde insuffisant ({wallet['balance']:.0f} Frs disponibles)."}
    if not selections:
        return {"placed": False, "reason": "Coupon vide."}

    conn = _connect()
    try:
        now = datetime.now().isoformat()
        cur = conn.execute(
            """INSERT INTO wallet_coupon_bets (user_id, engine, label, total_cote, stake, status, placed_at)
               VALUES (?,?,?,?,?, 'pending', ?)""",
            (user_id, engine, label, total_cote, stake, now),
        )
        coupon_bet_id = cur.lastrowid
        for sel in selections:
            conn.execute(
                """INSERT INTO wallet_coupon_legs (coupon_bet_id, match_id, home, away, league, prediction, cote, status)
                   VALUES (?,?,?,?,?,?,?, 'pending')""",
                (coupon_bet_id, str(sel.get("match_id") or sel.get("id") or ""), sel.get("home"),
                 sel.get("away"), sel.get("league"), sel.get("prediction"), sel.get("cote", 1.0)),
            )
        conn.execute(
            "UPDATE wallets SET balance = balance - ?, updated_at = ? WHERE user_id = ?",
            (stake, now, user_id),
        ...
        conn.commit()
        return {"placed": True, "new_balance": wallet["balance"] - stake, "coupon_bet_id": coupon_bet_id}
    finally:
        conn.close()


def settle_wallet_coupon_bets() -> dict:
    """Règle les coupons combinés en attente : chaque match (leg) est vérifié
    individuellement (même filet de résolution que le reste) ; le coupon
    entier n'est réglé (gagné/perdu) qu'une fois TOUS ses matchs terminés —
    gagné seulement si chacun d'eux était correct."""
    from coupon_tracker import _lookup_result
    ...
    try:
        ...
        pending_bets = conn.execute("SELECT * FROM wallet_coupon_bets WHERE status = 'pending'").fetchall()
        for bet in pending_bets:
            legs = conn.execute("SELECT * FROM wallet_coupon_legs WHERE coupon_bet_id = ?", (bet["id"],)).fetchall()
            ...
            for leg in legs:
                if leg["status"] != "pending":
                    if leg["status"] == "lost":
                        any_lost = True
                    continue
                result = _lookup_result(result_conn, leg["match_id"], leg["home"] or "", leg["away"] or "", "")
                if not result or not result.get("result"):
                    all_settled = False
                    continue
                leg_status = "won" if result["result"] == leg["prediction"] else "lost"
                conn.execute("UPDATE wallet_coupon_legs SET status = ? WHERE id = ?", (leg_status, leg["id"]))
                if leg_status == "lost":
                    any_lost = True

            now = datetime.now().isoformat()
            if any_lost:
                # Un seul match perdu suffit à perdre tout le coupon combiné —
                # inutile d'attendre les autres matchs pour le déclarer perdu.
                conn.execute(
                    "UPDATE wallet_coupon_bets SET status = 'lost', settled_at = ? WHERE id = ?",
                    (now, bet["id"]),
                )
                settled += 1
                lost += 1
            elif all_settled:
                # Tous les matchs sont connus et aucun n'est perdu -> gagné.
                payout = bet["stake"] * bet["total_cote"]
                conn.execute(
                    "UPDATE wallet_coupon_bets SET status = 'won', settled_at = ? WHERE id = ?",
                    (now, bet["id"]),
                )
                conn.execute(
                    "UPDATE wallets SET balance = balance + ?, updated_at = ? WHERE user_id = ?",
                ...
                settled += 1
                won += 1
            # sinon : encore des matchs pas terminés, on laisse en attente
...
def get_wallet_coupon_history(user_id: int, limit: int = 20) -> list[dict]:
    conn = _connect()
    try:
        bets = conn.execute(
            "SELECT * FROM wallet_coupon_bets WHERE user_id = ? ORDER BY placed_at DESC LIMIT ?",
            (user_id, limit),
        ).fetchall()
        out = []
        for b in bets:
            legs = conn.execute("SELECT * FROM wallet_coupon_legs WHERE coupon_bet_id = ?", (b["id"],)).fetchall()
            item = dict(b)
            item["legs"] = [dict(l) for l in legs]
            out.append(item)
        return out
    finally:
        conn.close()
    ...
    try:
        row = conn.execute(
            """SELECT COUNT(*) as total,
               ...
               FROM wallet_bets WHERE user_id = ?""",
            (user_id,),
        ).fetchone()
        coupon_row = conn.execute(
            """SELECT COUNT(*) as total,
                      ...
                      SUM(CASE WHEN status='lost' THEN 1 ELSE 0 END) as lost,
                      SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END) as pending
               FROM wallet_coupon_bets WHERE user_id = ?""",
            (user_id,),
        ).fetchone()

        won = (row["won"] or 0) + (coupon_row["won"] or 0)
        lost = (row["lost"] or 0) + (coupon_row["lost"] or 0)
        pending = (row["pending"] or 0) + (coupon_row["pending"] or 0)
        total = (row["total"] or 0) + (coupon_row["total"] or 0)
        settled = won + lost
        return {
            "balance": wallet["balance"],
            "initial_balance": wallet["initial_balance"],
            "pnl": wallet["balance"] - wallet["initial_balance"],
            "total_bets": total,
            "won": won,
            "lost": lost,
            "pending": pending,
            "win_rate": round(100 * won / settled, 1) if settled else None,
        }
    finally:
        conn.close()


def get_wallet_history(user_id: int, limit: int = 50) -> list[dict]:
    conn = _connect()
    try:
        rows = conn.execute(
            "SELECT * FROM wallet_bets WHERE user_id = ? ORDER BY placed_at DESC LIMIT ?",
            (user_id, limit),
        ).fetchall()
        return [dict(r) for r in rows]
    finally:
...
def get_wallet_leaderboard(limit: int = 20) -> list[dict]:
    """Classement des meilleurs gestionnaires de budget (solde le plus élevé)."""
    conn = _connect()
    try:
        rows = conn.execute(
            """SELECT u.pseudo, u.avatar_emoji, w.balance, w.initial_balance
