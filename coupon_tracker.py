...
def settle_pending_coupons(force_after_hours: float = 72.0) -> dict:
    """
    ...
    GARDE-FOU : si un coupon reste 'pending' depuis plus de `force_after_hours`
    (3 jours par défaut), il est réglé quand même avec ce qui a pu être
    résolu — un seul match introuvable (compétition trop obscure, quota
    API-Football épuisé ce jour-là, nom d'équipe non reconnu...) ne doit pas
    bloquer le coupon indéfiniment. Les matchs non résolus sont marqués
    "indéterminé" et EXCLUS du calcul (ni comptés comme bon, ni comme mauvais,
    ni inclus dans la cote totale — traités comme "annulés", pratique standard
    des bookmakers pour une sélection impossible à vérifier).
    """
    init_tables()
    conn = _connect()
    settled_count = 0
    ...
    try:
        ...
        for coupon_row in pending:
            ...
            age_hours = None
            if coupon_row["generated_at"]:
                try:
                    age_hours = (datetime.now() - datetime.fromisoformat(coupon_row["generated_at"])).total_seconds() / 3600
                except Exception:
                    pass

            force_now = unresolved and age_hours is not None and age_hours >= force_after_hours

            if unresolved and not force_now:
                still_pending += 1
                continue

            # Les matchs non résolus (après le délai de grâce) sont traités
            # comme "annulés" : ni bons ni mauvais, exclus de la cote totale.
            for m in unresolved:
                resolved.append({**m, "result": None, "correct": None, "voided": True})

            scored = [m for m in resolved if m.get("correct") is not None]
            hits = sum(1 for m in scored if m["correct"])
            total = len(scored)
            total_odds = 1.0
            for m in scored:
                if m["correct"] and m.get("cote"):
                    total_odds *= float(m["cote"])
            roi = round((total_odds - 1) if total and hits == total else (-1.0 if total else 0.0), 3)
            ...
            try:
                ...
                for m in resolved:
                    if m.get("voided"):
                        continue  # match non résolu : on ne peut rien en apprendre, ne pas fausser les stats
            ...
            for m in resolved:
                if m.get("voided"):
                    continue  # non résolu : pas un vrai échec de prédiction, ne pas journaliser
                if not m["correct"]:
                    conn.execute(
