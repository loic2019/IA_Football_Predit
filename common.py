...
from canonical_match import deduplicate_matches
...
def init_session_state():
    ...
    if "chat_messages" not in st.session_state:
        st.session_state.chat_messages = [
            {
                "role": "bot",
                "content": "👋 Bonjour ! Je suis Max, ton copilote CongoBet AI. Demande-moi les meilleurs pronostics du jour, une analyse de match, ou discute de tout ce qui te passe par la tête — je suis là comme un pote calé en foot, pas juste un menu de commandes.",
...
def is_logged_in() -> bool:
    """Source UNIQUE de vérité pour "l'utilisateur est-il connecté ?" —
    à utiliser PARTOUT plutôt que de réinventer la vérification page par
    page.

    CORRECTIF : avant ce correctif, l'appli avait TROIS vérifications
    différentes et incohérentes :
    - profil.py::_is_logged_in()   -> vérifiait seulement st.session_state.auth_user
    - challenge.py::_require_login() -> vérifiait seulement st.session_state.user_profile
    - communaute.py / app_dashboard.py -> vérifiaient les DEUX

    Si les deux clés de session se désynchronisaient ne serait-ce qu'un
    instant (ex: après une mise à jour de profil qui ne touche que l'une
    des deux), certaines pages considéraient l'utilisateur déconnecté
    alors que d'autres non — donnant l'impression aléatoire d'être "jeté"
    d'une page en particulier plutôt que d'une vraie déconnexion globale.

    user_profile est la source retenue : c'est ce qui est réellement
    affiché/utilisé partout dans l'appli (pseudo, avatar, id pour les
    opérations en base) — auth_user n'est qu'un détail interne de
    bookkeeping du token Firebase.
    """
    return bool(st.session_state.get("user_profile"))


def get_current_profile():
    """Retourne le profil utilisateur courant (dict), ou None si pas
    connecté. À utiliser à la place de st.session_state.user_profile
    directement, pour rester cohérent avec is_logged_in()."""
    return st.session_state.get("user_profile") if is_logged_in() else None
...
def get_matches_for_prediction(limit=200, include_recent_fallback=True):
    ...
    for match in live_matches + future_matches:
        ...
        # Garde-fou anti-pollution : un match sans AUCUNE cote réelle en base
        # (table `odds`) n'est jamais un vrai match Congobet/1xBet/Premierbet
        # scrapé — ça ne peut être qu'un résidu d'import historique externe
        # (ex: results_importer.py, données football-data.org) écrit par
        # erreur dans la même table `matches`. On l'exclut systématiquement :
        # un "pronostic" à cote 0.00 n'a aucun sens pour un vrai coupon.
        if not prepared.get("markets"):
            continue
    ...
    # Déduplication multi-sources (CongoBet + 1xBet + PremierBet écrivent
    # tous dans la même table matches, chacun avec ses propres IDs — voir
    # canonical_match.py). `seen` ci-dessus ne dédoublonne QUE par ID exact,
    # ce qui ne peut jamais attraper le même vrai match vu par deux sources
    # différentes. Sans ça, un même match pouvait apparaître deux fois dans
    # le même coupon, avec potentiellement deux cotes différentes.
    dedup_result = deduplicate_matches(prepared_matches)
    prepared_matches = dedup_result["matches"]
...
def save_automation_state(state):
    # Écriture atomique — même raison que predictor.ModelData.save() : évite
    # qu'une écriture concurrente (cycle auto en tâche de fond + action
    # manuelle) ne corrompe le fichier en coupant une écriture en plein
    # milieu.
    tmp_path = AUTOMATION_STATE_PATH.with_suffix(AUTOMATION_STATE_PATH.suffix + ".tmp")
    with open(tmp_path, "w", encoding="utf-8") as f:
        json.dump(state, f, ensure_ascii=False, indent=2)
    os.replace(tmp_path, AUTOMATION_STATE_PATH)
...
def run_prediction_pipeline(limit=200, min_confidence=0.0, min_cote=1.30):
    ...
    # Ticket secondaire de 15 matchs, seuils volontairement plus souples que
    # le coupon 8 (sinon il n'y aurait jamais assez de sélections pour en
    # remplir 15) — même logique que le CLI `predictor.py --coupon2 15`.
    coupon15 = None
    if len(predictions) >= 15:
        coupon15 = predictor.build_coupon(
            predictions, size=15, min_confidence=min(min_confidence, 0.35),
            min_cote=min(min_cote, 1.15), label="Ticket 15 matchs",
        )

    # Plusieurs coupons de 10 matchs, du plus sûr au plus risqué (répartit
    # TOUS les matchs valables plutôt que de n'en garder que les 8 meilleurs).
    multi_coupons = predictor.build_multiple_coupons(predictions, group_size=10)
...
def run_training_pipeline(limit=500):
    ...
    - + les résultats historiques football-data.org (historical_results.db),
      ...
      calibration du modèle. Alimenté ACTIVEMENT (pas juste une capture figée
      une fois) via historical_data_refill.py, avec un plafond rotatif de
      10 000 matchs : au-delà, les plus anciens sont supprimés automatiquement
      pour laisser la place aux plus récents.
    Les deux sources ne partagent jamais le même schéma ni la même base :
    aucun risque d'écrasement.
    """
    from predictor import Predictor, parse_match_datetime
    from historical_data_refill import refill_if_due

    try:
        refill_if_due(min_hours=24)  # ne tape l'API que si pas déjà fait dans les dernières 24h
    except Exception:
        pass  # un souci réseau/API ne doit jamais bloquer l'entraînement
    ...
    # CRITIQUE (fuite de données) : get_finished_matches, get_recent_matches_
    # with_results et get_historical_training_matches trient TOUS en
    # ORDER BY ... DESC (plus récent en premier), et étaient jusqu'ici
    # simplement concaténés sans retri. Or team_form (ModelData.team_form)
    # est un état MUTABLE mis à jour au fil de train_from_results() : sans
    # ce tri chronologique croissant, la "forme" utilisée pour prédire un
    # vieux match pouvait déjà intégrer des résultats de matchs plus
    # récents traités avant lui dans la boucle — le modèle "savait" déjà
    # l'avenir de l'équipe au moment de prédire son passé.
    # Vérifié et quantifié : sur un cas de test simple, cette fuite gonflait
    # une probabilité de 0.2673 à 0.2923 pour le tout premier match d'une
    # équipe, uniquement à cause de deux victoires FUTURES déjà connues.
    combined.sort(key=lambda m: parse_match_datetime(m.get("start_time")) or datetime.min.replace(tzinfo=timezone.utc))

    report = predictor.train_from_results(combined, limit=len(combined))

    # Entraînement continu du réseau de neurones interne (13->64->32->3,
    # ~3075 poids) sur les features accumulées dans l'historique par l'appel
    # ci-dessus. Peu d'époques par cycle (les poids sont rechargés depuis
    # nn_weights.json à chaque Predictor() : l'apprentissage s'accumule d'un
    # cycle à l'autre, ce n'est jamais repris à zéro). max_samples borne le
    # temps de calcul même quand l'historique global atteint son plafond
    # (100 000 matchs) — un souci ici ne doit jamais faire échouer le cycle.
    try:
        report["neural_net"] = predictor.train_neural_net(epochs=10, max_samples=5000)
    except Exception as exc:
        report["neural_net"] = {"status": "error", "message": str(exc)}
...
def run_auto_cycle(force=False, include_besoccer=False, include_premierbet=True):
    ...
    try:
        ...
        try:
            ...
            # --- Règlement des paris virtuels (portefeuilles utilisateurs,
            # page Challenge) — même filet de résolution que les vrais coupons. ---
            try:
                import community_db
                wallet_settle = community_db.settle_wallet_bets()
                wallet_coupon_settle = community_db.settle_wallet_coupon_bets()
                summary["wallet_settle"] = wallet_settle
                summary["wallet_coupon_settle"] = wallet_coupon_settle
            except Exception as exc:
                summary["wallet_settle"] = f"erreur: {exc}"
