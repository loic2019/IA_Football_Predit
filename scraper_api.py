...
def _clean_team_name(name: str) -> str:
    """Congobet préfixe parfois le nom d'équipe d'un code de compétition
    court suivi d'un point (ex: 'ZWC.Morocco' au lieu de 'Morocco') pour
    certains tournois. Ce préfixe casse la correspondance de noms avec les
    sources externes (API-Football) lors du règlement des coupons — on le
    retire ici, à la source, pour que TOUT en aval (affichage, entraînement,
    recherche de résultat) utilise un nom propre."""
    if not name:
        return name
    match = re.match(r"^([A-Z]{2,6})\.(.+)$", name)
...
def parse_event(event: dict) -> dict | None:
    ...
    try:
        home = _clean_team_name(event.get("homeTeamName", ""))
        away = _clean_team_name(event.get("awayTeamName", ""))
