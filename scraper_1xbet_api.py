...
GET_1X2_VZIP = (
    ...
    + "&top=true&noFilterBlockEvent=true"
)

# Mots-clés identifiant du football VIRTUEL/simulé (Esoccer, Cyber Football,
# FIFA/eFootball...) — 1xBet les classe parfois dans les mêmes flux que le
# vrai football. Le paramètre `virtualSports=true` retiré ci-dessus était la
# cause principale (il demandait explicitement à l'API d'inclure ces
# événements) ; ce filtre par nom de ligue est une seconde protection, au cas
# où l'API en renverrait quand même. Sans ces deux protections, des matchs
# qui n'ont JAMAIS eu lieu dans la réalité (générés par ordinateur) entraient
# dans congobet.db exactement comme du vrai football — indétectables comme
# faux une fois en base, puisqu'ils ont un score, une date et des cotes tout
# à fait normaux en apparence.
_VIRTUAL_LEAGUE_KEYWORDS = (
    "virtual", "cyber", "esoccer", "e-soccer", "efootball", "e-football",
    "fifa", "battle",
)


def _is_virtual_league(league: str) -> bool:
    league_lower = (league or "").lower()
    return any(kw in league_lower for kw in _VIRTUAL_LEAGUE_KEYWORDS)
...
def scrape_1xbet_top_events(count: int = 30) -> list[dict]:
    ...
    if skipped_virtual:
        log.info(f"[FILTRE] {skipped_virtual} evenement(s) de football virtuel/eSport ecarte(s)")
