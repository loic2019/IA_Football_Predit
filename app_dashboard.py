...
# NE PAS ré-envelopper sys.stdout/sys.stderr ici (contrairement aux scripts
# CLI comme predictor.py/run.py, où c'est nécessaire pour éviter un
# UnicodeEncodeError sur une console Windows en cp1252).
#
# Dans une app Streamlit, c'est la cause du bug "I/O operation on closed
# file" observé un peu partout (voir le commentaire dans challenger_model.py,
# qui l'avait déjà identifié comme cause probable) : Streamlit exécute et
# ré-exécute ce script dans son propre thread à chaque interaction, et gère
# lui-même l'encodage de sortie (l'UI passe par le navigateur, pas par la
# console). Remplacer sys.stdout par un TextIOWrapper global — un objet
# persistant tenant un verrou sur le buffer sous-jacent — entre en conflit
# avec ce cycle de ré-exécution : un module qui écrit encore sur l'ancien
# flux (ex. la barre de progression de huggingface_hub dans
# challenger_model.py) peut tomber sur un flux déjà fermé par un rerun
# précédent. Les emojis dans st.markdown/st.title ne posent aucun problème :
# ils sont rendus par le navigateur, pas imprimés sur la console.
try:
    pass  # volontairement neutralisé — voir commentaire ci-dessus
except Exception:
    pass
...
SIDEBAR_PAGES = ["Chatbot IA", "Historique", "Statistiques", "Dashboard", "Backtesting", "Challenge", "Fichiers", "Paramètres", "Profil"]
...
# Filet de sécurité : dès l'ouverture de l'app (nouvelle session Streamlit),
# on vérifie si un cycle est dû et, si oui, on le lance en arrière-plan
# (thread démon, ne bloque pas l'affichage) — indépendamment du worker externe
# (auto_cycle_worker.py / tâche planifiée Windows). Une seule fois par session
# (pas à chaque interaction) ; run_auto_cycle gère déjà lui-même le cas où ce
# n'est pas encore dû (retour immédiat, aucun risque de double cycle).
if "_auto_cycle_kickstarted" not in st.session_state:
    st.session_state._auto_cycle_kickstarted = True
    import threading
    from common import run_auto_cycle
    threading.Thread(target=lambda: run_auto_cycle(force=False), daemon=True).start()
