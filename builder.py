...
def _form_score_from_list(form: list) -> float:
    """Reproduit exactement predictor.ModelData.get_team_form_score, mais à
    partir d'une liste W/D/L arbitraire — permet de calculer un score de
    forme sur un historique "rejoué" (voir build_training_matrix) plutôt que
    sur l'état live de model.data['team_form']."""
    if not form:
        return 0.5
    points = sum(1.0 if v == "W" else 0.5 if v == "D" else 0.0 for v in form)
    return points / len(form)


def _get_team_form_list(team: str, model, form_source: dict | None) -> list:
    """Point d'accès UNIQUE à l'historique de forme d'une équipe dans tout ce
    fichier. form_source=None (par défaut) -> comportement inchangé, lit
    l'état live de model.data['team_form'] (correct pour une VRAIE
    prédiction, où on veut justement toute la connaissance accumulée).
    form_source=dict -> lit ce dict à la place (utilisé uniquement par
    build_training_matrix, voir plus bas, pour un rejeu chronologique)."""
    if form_source is not None:
        return form_source.get(team, [])
    return model.data.get("team_form", {}).get(team, [])


def _team_form_extended(team: str, model, window: int, form_source: dict | None = None) -> dict[str, float]:
    """
    ...
    Args:
        ...
        form_source: Voir _get_team_form_list.

    Returns:
        Dict avec win_rate, draw_rate, loss_rate, goals_scored, goals_conceded, points_per_game.
    """
    form = _get_team_form_list(team, model, form_source)[-window:]
    if not form:
        base = _form_score_from_list(_get_team_form_list(team, model, form_source))
        return {
            "win_rate": base * 0.6,
            "draw_rate": 0.25,
...
def build_legacy_features(match: dict, model, form_source: dict | None = None) -> dict[str, float]:
    """
    ...
    home_form = _form_score_from_list(_get_team_form_list(home, model, form_source))
    away_form = _form_score_from_list(_get_team_form_list(away, model, form_source))
...
def build_extended_features(match: dict, model, legacy: dict[str, float], form_source: dict | None = None) -> dict[str, float]:
    """
    ...
    for side, team in (("home", home), ("away", away)):
        for w in (3, 5, 10):
            stats = _team_form_extended(team, model, w, form_source=form_source)
    ...
    for side, team, form in (
        ...
        s5 = _team_form_extended(team, model, 5, form_source=form_source)
        s10 = _team_form_extended(team, model, 10, form_source=form_source)
...
def build_feature_vector(match: dict, model, extended: bool = True, form_source: dict | None = None) -> np.ndarray:
    """
    ...
    Args:
        ...
        form_source: Voir _get_team_form_list. None (par défaut) = comportement
            normal, lit la forme live du modèle — c'est ce qu'il faut pour une
            VRAIE prédiction (on veut toute la connaissance accumulée). Utilisé
            avec un dict explicite uniquement par build_training_matrix, pour
            rejouer la forme de façon chronologiquement correcte à l'entraînement.

    Returns:
        np.ndarray float32 de shape (n_features,).
    """
    legacy_dict = build_legacy_features(match, model, form_source=form_source)
    if not extended:
        return np.array([legacy_dict[k] for k in LEGACY_FEATURE_NAMES], dtype=np.float32)

    ext_dict = build_extended_features(match, model, legacy_dict, form_source=form_source)
...
def build_training_matrix(matches: list[dict], model, extended: bool = False) -> tuple[np.ndarray, np.ndarray, list[dict]]:
    ...
    CORRECTIF FUITE TEMPORELLE : cette fonction est appelée par
    ensemble.orchestrator.train_ensemble(matches, model) APRÈS que la boucle
    Poisson principale (predictor.Predictor.train_from_results) ait déjà mis
    à jour model.data['team_form'] pour la TOTALITÉ du lot `matches`. Avant ce
    correctif, chaque feature vector était construit avec
    model.get_team_form_score(team), qui lisait cet état déjà "complet" —
    chaque exemple d'entraînement XGBoost/LightGBM/CatBoost/etc. connaissait
    donc déjà l'issue de matchs plus récents du même lot. Fuite distincte de
    celle corrigée dans common.py::run_training_pipeline (qui ne protégeait
    que la boucle Poisson elle-même, pas cette matrice ML séparée).

    Le correctif rejoue la forme de zéro en avançant chronologiquement dans
    `matches` : chaque exemple utilise uniquement les résultats des matchs
    qui le PRÉCÈDENT dans ce même lot, jamais ceux qui le suivent. Coût
    assumé : l'historique de forme accumulé AVANT ce lot (dans model_data.json,
    via les cycles précédents) n'est pas utilisé ici — seulement ce qui est
    visible à l'intérieur de `matches` lui-même. Sur un lot de plusieurs
    centaines/milliers de matchs (cas réel via historical_data), l'essentiel
    des équipes a largement assez d'historique intra-lot pour que ça reste
    représentatif.

    Args:
        matches: Matchs terminés avec résultat (PAS besoin d'être pré-triés :
            cette fonction trie elle-même chronologiquement, comme
            backtesting.engine.walk_forward_validation le fait déjà).
        model: ModelData.
    ...
    def sort_key(m):
        return m.get("start_time") or m.get("utc_date") or m.get("date") or ""

    ordered = sorted(matches, key=sort_key)

    # Rejeu local, indépendant de model.data['team_form'] — voir docstring.
    replay_form: dict[str, list] = {}

    def touch(team: str, result_letter: str):
        replay_form.setdefault(team, []).append(result_letter)
        replay_form[team] = replay_form[team][-10:]
    ...
    for match in ordered:
        ...
        if not label_str or label_str not in RESULT_TO_LABEL:
            continue
        try:
            features = build_feature_vector(match, model, extended=extended, form_source=replay_form)
        except Exception:
            continue
        ...
        # Mise à jour du rejeu APRÈS extraction des features de ce match —
        # jamais avant (sinon on retomberait dans le même problème).
        home, away = match.get("home", ""), match.get("away", "")
        if label_str == "1":
            touch(home, "W"); touch(away, "L")
        elif label_str == "2":
            touch(home, "L"); touch(away, "W")
        else:
            touch(home, "D"); touch(away, "D")
