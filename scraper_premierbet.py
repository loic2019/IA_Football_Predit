...
def run_once(days_ahead: int = DEFAULT_DAYS_AHEAD) -> dict:
    # Ces print() sont purement décoratifs (bannière console). Sur Windows,
    # exécutés dans le thread-pool d'Uvicorn (--reload), le flux stdout peut
    # occasionnellement être fermé/invalide (spécifique au subprocess de
    # rechargement), ce qui faisait planter TOUT le scraping avec une erreur
    # 500 alors que la logique elle-même fonctionne très bien. On les protège
    # pour que l'échec d'un simple print() n'empêche jamais le scraping réel.
    try:
        print(f"\n{'═'*55}")
