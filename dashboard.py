...
def render():
    ...
    # Réutilise directement pronostics.load_predictions (même cache, mêmes
    # données) plutôt que de dupliquer l'appel à predictor.py — un seul appel
    # à run_prediction_pipeline() par cycle de cache (ttl=30s), partagé entre
    # les deux pages.
    st.markdown("### 🎟️ Coupon du jour (aperçu)")
    try:
        from pronostics import load_predictions

        snapshot = load_predictions(limit=100, min_confidence=0.0, min_cote=1.30)
        coupon = snapshot.get("coupon") or {}
        if not coupon.get("selections"):
            st.caption("Aucune sélection disponible pour l'instant — voir la page Pronostics.")
        else:
            cc1, cc2, cc3, cc4 = st.columns(4)
            cc1.metric("Sélections", coupon.get("size", 0))
            cc2.metric("Cote totale", f"{coupon.get('total_cote', 0):.2f}")
            cc3.metric("Confiance moy.", f"{coupon.get('avg_confidence', 0):.0%}")
            value_count = coupon.get("value_bet_count", 0)
            cc4.metric("Value bets", f"{value_count}/{coupon.get('size', 0)}")
            if value_count == 0:
                st.caption("⚠️ Aucun edge statistique détecté sur ce coupon — voir la page Pronostics pour le détail.")
        if st.button("Voir tous les pronostics →", key="dash_goto_pronostics"):
            from common import goto_page
            goto_page("Pronostics")  # déclenche déjà son propre st.rerun()
    except Exception as e:
        st.caption(f"Aperçu du coupon indisponible : {e}")
    ...
    # Lu directement depuis nn_weights.json via predictor.NeuralNet — ne dépend
    # pas de monitoring/metrics.py, donc s'affiche même si ce module n'a pas
    # encore été mis à jour pour l'intégrer.
    st.markdown("### 🧬 Réseau de neurones interne")
    try:
        from predictor import NeuralNet, NN_WEIGHTS_PATH, NN_INPUT_SIZE, NN_HIDDEN1, NN_HIDDEN2, NN_BLEND_WEIGHT

        nn = NeuralNet.load(NN_WEIGHTS_PATH)
        if nn is None or nn.trained_epochs == 0:
            st.info(
                "Pas encore entraîné — il n'influence pas encore les pronostics. "
                "Lance \"Entraîner le réseau de neurones\" depuis la page Fichiers & outils, "
                "ou attends le prochain cycle automatique (il s'entraîne désormais à chaque cycle)."
            )
        else:
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Poids entraînables", f"{nn.total_weights():,}".replace(",", " "))
            c2.metric("Époques cumulées", nn.trained_epochs)
            c3.metric("Accuracy interne", f"{nn.last_accuracy:.1%}" if nn.last_accuracy is not None else "N/A")
            c4.metric("Poids dans le blend", f"{NN_BLEND_WEIGHT:.0%}")
            st.caption(
                f"Architecture {NN_INPUT_SIZE} → {NN_HIDDEN1} → {NN_HIDDEN2} → 3 · "
                f"dernier entraînement sur {nn.trained_samples} matchs · loss {nn.last_loss}"
            )
    except Exception as e:
        st.caption(f"Statut du réseau de neurones indisponible : {e}")
    ...
    try:
        ...
        if real_history:
            rows = [{
                ...
                "Bons pronostics": str(h.get("hits")) if h.get("hits") is not None else "—",
    ...
    # === Compteur du jeu de calibration (historical_results.db, plafond 10 000) ===
    from historical_data_refill import get_stats as get_historical_stats, refill as refill_historical

    hstats = get_historical_stats()
    total_hist = hstats["total"]
    capacity = hstats["capacity"]
    last_at = hstats.get("last_import_at")
    last_count = hstats.get("last_import_count", 0)

    st.markdown("### 📥 Jeu de calibration (matchs d'entraînement)")
    c1, c2, c3 = st.columns(3)
    c1.metric("Matchs historiques en base", f"{total_hist:,} / {capacity:,}".replace(",", " "))
    c2.metric("Ajoutés au dernier import", f"+{last_count}")
    last_display = last_at[:16].replace("T", " ") if last_at else "jamais encore"
    c3.metric("Dernier import", last_display)
    st.progress(min(1.0, total_hist / capacity))
    st.caption(
        "Une fois le plafond de 10 000 atteint, les matchs les plus anciens sont "
        "automatiquement supprimés pour laisser la place aux plus récents (rotation "
        "systématique) — le modèle reste calibré sur des données représentatives, "
        "actualisé automatiquement dans le cycle (max 1 fois/24h pour respecter l'API gratuite)."
    )
    if st.button("📡 Lancer un import maintenant", key="manual_historical_refill"):
        with st.spinner("Récupération des derniers résultats des grandes ligues..."):
            result = refill_historical()
        st.success(f"✅ {result['added']} nouveau(x) match(s) ajouté(s) — total : {result['total']:,}".replace(",", " "))
        st.rerun()
